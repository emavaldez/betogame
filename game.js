// =============================================
// LA VIDA DE BETO - Motor Principal
// =============================================
...
class Beto extends Entity {
    ...
    createModel() {
        ...
        // Camiseta amarilla sobre el azul
    ...
    move(direction) {
        ...
        if (direction.includes('w') || direction.includes('arrowup')) {
            moveVector.z -= 1;
        }
        if (direction.includes('s') || direction.includes('arrowdown')) {
            moveVector.z += 1;
        }
        if (direction.includes('a') || direction.includes('arrowleft')) {
            moveVector.x -= 1;
        }
        if (direction.includes('d') || direction.includes('arrowright')) {
            moveVector.x += 1;
        }
...
class Car extends Entity {
    constructor(scene, color = null) {
        ...
        this.state = 'waiting'; // waiting, parking, parked, leaving
        this.parkPosition = null;
        this.arrivalTime = Date.now();
        this.maxWaitTime = 15000; // 15 segundos para ser guiado
    ...
    createModel() {
        ...
        // Texto "Click" flotante
        const textGeo = new THREE.PlaneGeometry(1.5, 0.6);
        this.clickTextMaterial = new THREE.MeshBasicMaterial({
            color: 0xFFFFFF,
            transparent: true,
            opacity: 0.8
        });
    ...
    update(deltaTime) {
        ...
        switch (this.state) {
            ...
            case 'parking':
                // Animación de estacionamiento
                const parkProgress = Math.min(
                    (Date.now() - this.parkStartTime) / 1000,
                    1
                );

                if (this.parkPosition) {
                    // Interpolar hacia la posición de estacionamiento
                ...
                if (parkProgress >= 1) {
                    ...
                    // Generar pasajero
                    const passenger = new Passenger(this.scene);
                    passenger.moveTo(0.5, -3);
                    GAME.entities.passengers.push(passenger);
                }
                break;
        ...
        this.mesh.position.y = 0.5; // Mantener en el suelo
    }
...
class Passenger extends Entity {
    constructor(scene) {
        super(scene);
        this.state = 'waiting'; // waiting, paying, leaving
        this.arrivalTime = Date.now();
        this.maxWaitTime = 8000; // 8 segundos para cobrar

        this.createModel();

        // Randomizar apariencia (accesorios de Boca)
        this.accessories = [];
        const possibleAccessories = ['remera', 'pantalon', 'bufanda', 'gorro', 'trompeta'];
        const numAcc = Math.floor(Math.random() * 3) + 1; // 1-3 accesorios

        for (let i = 0; i < numAcc; i++) {
            const acc = possibleAccessories[Math.floor(Math.random() * possibleAccessories.length)];
    ...
    createModel() {
        ...
        // Texto "Pagar"
        const textGeo = new THREE.PlaneGeometry(1.5, 0.6);
        this.payTextMaterial = new THREE.MeshBasicMaterial({
            color: 0xFFFFFF,
            transparent: true,
            opacity: 0.8
        });
        ...
        this.accessories.forEach(acc => {
            ...
            switch (acc) {
                case 'remera':
                    ...
                    const remeraMat = new THREE.MeshStandardMaterial({
                ...
                case 'bufanda':
                    ...
                    const bufMat = new THREE.MeshStandardMaterial({
                        color: colors[Math.floor(Math.random() * colors.length)]
                    });
                ...
                case 'gorro':
                    ...
                    const gorroMat = new THREE.MeshStandardMaterial({
                        color: colors[Math.floor(Math.random() * colors.length)],
                        roughness: 0.9
                    });
    ...
    update(deltaTime) {
        ...
        switch (this.state) {
            ...
            case 'paying':
                const payProgress = Math.min(
                    (Date.now() - this.payStartTime) / 1000,
                    1
                );

                if (payProgress >= 1) {
                    this.state = 'leaving';
                    this.payTextMaterial.color.setHex(0x4CAF50); // Verde de éxito
                }
                break;
        ...
        this.mesh.position.y = 1; // Mantener en el suelo
    }
...
class Dog extends Entity {
    constructor(scene) {
        super(scene);
        this.state = 'wandering'; // wandering, attacking, leaving
        this.attackTime = 0;
    ...
    update(deltaTime) {
        ...
        switch (this.state) {
            case 'wandering':
                ...
                this.mesh.children[3].position.y = 0.4 + walkOffset; // Pata delantera
                this.mesh.children[6].position.y = 0.4 - walkOffset; // Pata trasera

                break;

            case 'attacking':
                const attackProgress = Math.min(
                    (Date.now() - this.attackTime) / 1500,
                    1
                );
...
class GameEngine {
    ...
    createParkingLines() {
        ...
        for (let i = -40; i <= 40; i += 6) {
            ...
            line.position.set(i, 0.1, -15);
            this.scene.add(line);

            // Líneas horizontales
            const horizontalLine = new THREE.Mesh(lineGeo, lineMat);
            horizontalLine.rotation.x = -Math.PI / 2;
            horizontalLine.rotation.y = Math.PI / 2;
            horizontalLine.position.set(i, 0.1, -12);
    ...
    onCanvasClick(e) {
        ...
        // Raycaster para detectar clics en el suelo
        const raycaster = new THREE.Raycaster();
        raycaster.setFromCamera(new THREE.Vector2(x, y), this.camera);

        const ground = this.scene.children.find(c => c.geometry.type === 'PlaneGeometry' && c.material.color.getHex() === CONFIG.colors.grass);
    ...
    startLevel(level) {
        ...
        const spawnInterval = setInterval(() => {
            ...
            if (GAME.entities.cars.length < GAME.maxCars) {
                const car = new Car(this.scene);

                // Posición aleatoria al inicio
        ...
        // Guardar intervals para limpiar
